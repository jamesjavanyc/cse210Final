...
class KeyboardService(KeyboardInterface):
    ...
    def get_dir(self, cast):
        ...
        if pyray.is_key_down(pyray.KEY_W) and cast["game"].player.bullet >= 1:
            bullet = Bullet()
            bullet.set_text(".")
            bullet.set_position(Point(cast["game"].player.get_position().get_x(), cast["game"].player.get_position().get_y() + 20))
            cast["bullet"].append(bullet)
            cast["game"].player.bullet -= 1
            game = cast["game"]
